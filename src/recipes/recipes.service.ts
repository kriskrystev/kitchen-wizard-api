...
export class RecipesService {
     ...
     */
    async create(createRecipeDto: CreateRecipeDto, userEmail: string): Promise<ReadRecipeDto> {
        const user = await this.usersService.findUserByEmail(userEmail);
        const recipeExists = await this.recipeModel.findOne({ title: createRecipeDto.title });
        if (recipeExists) {
     ...
     */
    async update(id: string, updateRecipeDto: UpdateRecipeDto): Promise<ReadRecipeDto> {
        const recipeEntity = await this.recipeModel.findByIdAndUpdate(id, updateRecipeDto, { new: true });
     ...
     */
    async createIngredient(recipeId: string, createIngredientDto: CreateIngredientDto): Promise<ReadRecipeDto> {
    ...
    async findIngredients(id: string): Promise<CreateIngredientDto[]> {
        ...
        const mappedRecipe = await this.classMapper.mapAsync(recipe, Recipe, ReadRecipeDto);
    ...
    async findIngredientById(recipeId: string, ingredientId: string) {
        ...
        return await this.classMapper.mapAsync(ingredients[0], Ingredient, ReadIngredientDto);
    }
    ...
    async removeIngredient(recipeId: string, ingredientId: string) {
        ...
        return await this.classMapper.mapArrayAsync(ingredients, Ingredient, ReadIngredientDto);
    }
     ...
     * @throws {IngredientNotFoundException, RecipeNotFoundException}
     */
    async updateIngredient(recipeId: string, ingredientId: string, updateIngredientDto: UpdateIngredientDto) {
        const recipe = await this.recipeModel.findById(recipeId);
